...
class ConsoleLog(base.APIBase):
    """API representation of the console log for a node."""

    content_type = wtypes.text
    """The content type: Currently, 'text/plain' is the only use case."""

    content = wtypes.text
    """The content in a log file of a node"""

    @classmethod
    def sample(cls):
        return cls(
            content_type='text/plain',
            content='Initializing cgroup subsys cpuset\nInitializing ...'
        )


class NodeConsoleLogController(rest.RestController):

    @expose.expose(ConsoleLog, types.uuid_or_name)
    def get(self, node_ident):
        """Get the content of a console log file of a node.
        ...
        topic = pecan.request.rpcapi.get_topic_for(rpc_node)
        try:
            console_log = pecan.request.rpcapi.get_console_log(
                pecan.request.context, rpc_node.uuid, topic)
        except exception.NodeConsoleNotEnabled:
            raise

        return ConsoleLog(context_type=console_log['content-type'],
                          content=console_log['content'])

    @expose.expose(None, types.uuid_or_name,
                   status_code=http_client.ACCEPTED)
    def delete(self, node_ident):
        """Clear the content of a console log file of a node.

        :param node_ident: UUID or logical name of a node.
        """
        rpc_node = api_utils.get_rpc_node(node_ident)
        topic = pecan.request.rpcapi.get_topic_for(rpc_node)
        pecan.request.rpcapi.clear_cosnole_node(pecan.request.context,
                                                rpc_node.uuid, topic)
...
class NodeStatesController(rest.RestController):
    ...
    console_log = NodeConsoleLogController()
