...
class ConductorManager(base_manager.BaseConductorManager):
    ...
    def _do_takeover(self, task):
        ...
        # as well.
        try:
            task.driver.console.start_console_log(task)
        except Exception as err:
            msg = (_('Failed to start console logging while taking '
                     'over the node %(node)s: %(err)s.') % {
                         'node': task.node.uuid, 'err': err})
            LOG.error(msg)
            # If taking over console logging failed, set node's
            # console_enabled back to False and set node's last error.
    ...
    def destroy_node(self, context, node_id):
        ...
        with task_manager.acquire(context, node_id,
            ...
            try:
                task.driver.console.stop_console_log(task)
            except Exception as err:
                msg = _LE('Failed to stop console log while taking over the '
                          'node %(node)s: %(err)s.')
                LOG.error(msg, {'node': task.node.uuid, 'err': err})
                task.node.last_error = msg
            node.destroy()
    ...
    def get_console_log(self, context, node_id):
        """Get content from the console log file of the node.
        ...
        :raises: MissingParameterValue if missing supplied info.
        """
        LOG.debug('RPC get_console_log called for node %s' % node_id)

        lock_purpose = 'getting console log'
        with task_manager.acquire(context, node_id, shared=True,
                                  purpose=lock_purpose) as task:
            node = task.node
            ...
            if not getattr(task.driver, 'console', None):
                raise exception.UnsupportedDriverExtension(driver=node.driver,
                                                           extension='console')
            return task.driver.console.get_console_log(task)
    ...
    @messaging.expected_exceptions(exception.NodeLocked,
                                   ...
                                   exception.InvalidParameterValue,
                                   exception.MissingParameterValue)
    def clear_console_log(self, context, node_id):
        """Clear content from the console log file of the node.
        ...
        :raises: MissingParameterValue if missing supplied info.
        """
        LOG.debug('RPC clear_console_log called for node %s' % node_id)

        lock_purpose = 'clearing console log'
        with task_manager.acquire(context, node_id, shared=False,
                                  purpose=lock_purpose) as task:
            node = task.node
            ...
            if not getattr(task.driver, 'console', None):
                raise exception.UnsupportedDriverExtension(driver=node.driver,
                                                           extension='console')
            task.driver.console.clear_console_log(task)
    ...
    try:
        ...
        if new_state == states.DEPLOYDONE:
            ...
            try:
                ...
                msg = (_('Failed to start console logging while taking over '
                         'the node %(node)s: %(err)s.') % {
                             'node': task.node.uuid,
                             'err': err})
                LOG.error(msg)
