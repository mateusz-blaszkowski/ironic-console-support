...
class IPMIIcsConsole(base.ConsoleInterface):
    """A ConsoleInterface that uses ipmitool and ironic console server."""
    ...
    def start_console_log(self, task):
        """Start console logging for the node, i.e. run a console server.
        ...
        ipmi_cmd += " sol activate"
        try:
            console_utils.start_ics_console_log(driver_info['uuid'],
                                                driver_info['port'],
                                                ipmi_cmd,
                                                task.node.console_enabled)
        except (exception.ConsoleError, exception.ConsoleSubprocessFailed):
            with excutils.save_and_reraise_exception():
                ironic_utils.unlink_without_raise(path)

    def stop_console_log(self, task):
        """Stop the console logging for the node, i.e. kill the console server.
        ...
        driver_info = _parse_driver_info(task.node)
        try:
            console_utils.stop_ics_console_log(driver_info['uuid'])
        finally:
            ironic_utils.unlink_without_raise(
                _console_pwfile_path(driver_info['uuid']))

    def get_console_log(self, task):
        """Get the type and content from the console log file."""
        driver_info = _parse_driver_info(task.node)
        console_log = console_utils.get_ics_console_log(driver_info['uuid'])
        return {'content-type': 'text/plain', 'content': console_log}

    def clear_console_log(self, task):
        ...
        """
        driver_info = _parse_driver_info(task.node)
        console_utils.clear_ics_console_log(driver_info['uuid'])
    ...
    def start_console(self, task):
        ...
        """
        driver_info = _parse_driver_info(task.node)
        console_utils.start_ics_console(driver_info['uuid'])
    ...
    def stop_console(self, task):
        ...
        """
        driver_info = _parse_driver_info(task.node)
        console_utils.stop_ics_console(driver_info['uuid'])

    def get_console(self, task):
        """Get the type and connection information about the console."""
        driver_info = _parse_driver_info(task.node)
        url = console_utils.get_ics_console_url(driver_info['port'])
        return {'type': 'ics', 'url': url}


def get_console_driver():
    """Return an object of console driver.

    :return: IPMIIcsConsole or IPMIShellinaboxConsole
    """
    if "ironic-console-server" in CONF.console.terminal:
        return IPMIIcsConsole()
    else:
        return IPMIShellinaboxConsole()
