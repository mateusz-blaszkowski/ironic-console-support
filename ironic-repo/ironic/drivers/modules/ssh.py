...
class SSHIcsConsole(base.ConsoleInterface):
    """A ConsoleInterface that uses ssh and ironic console server."""
    ...
    def start_console_log(self, task):
        ...
        ssh_cmd = ("virsh console %s" % node_name)
    ...
    def stop_console(self, task):
        ...
        console_utils.stop_ics_console(task.node.uuid)
    ...
    def get_console(self, task):
        ...
        driver_info = _parse_driver_info(task.node)
        url = console_utils.get_ics_console_url(driver_info['terminal_port'])
...
def get_console_driver():
    ...
    :returns: SSHIcsConsole() or ShellinaboxConsole()
    """
    if "ironic-console-server" in CONF.console.terminal:
        return SSHIcsConsole()
    else:
        return ShellinaboxConsole()
